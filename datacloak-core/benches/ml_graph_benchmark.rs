...
fn benchmark_column_ranking(c: &mut Criterion) {
    ...
    c.bench_function("rank_20_columns_ml_only", |b| {
        b.iter(|| integration.rank_columns_with_graph(black_box(&columns), 1.0))
    });

    c.bench_function("rank_20_columns_hybrid", |b| {
        b.iter(|| integration.rank_columns_with_graph(black_box(&columns), 0.7))
    });

    c.bench_function("rank_20_columns_graph_heavy", |b| {
        b.iter(|| integration.rank_columns_with_graph(black_box(&columns), 0.3))
    });
}
...
fn benchmark_graph_building(c: &mut Criterion) {
    ...
    let columns: Vec<Column> = (0..50)
        .map(|i| Column::new(&format!("column_{}", i), vec!["sample", "data", "values"]))
        .collect();

    c.bench_function("build_graph_50_columns", |b| {
        b.iter(|| integration.build_column_graph(black_box(&columns)))
    });
}
