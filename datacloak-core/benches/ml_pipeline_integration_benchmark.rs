...
use datacloak_core::{
    ml_classifier::{MLClassifier, Column},
...
fn benchmark_full_pipeline(c: &mut Criterion) {
    ...
    for column_count in [10, 50, 100, 500].iter() {
        let columns: Vec<Column> = (0..*column_count).map(|i| {
            if i % 3 == 0 {
                ...
                Column::new(&format!("num_col_{}", i), vec![
                    "123.45", "67.89", "999.00", "12.34"
                ])
            } else {
                Column::new(&format!("id_col_{}", i), vec![
                    "ID12345", "USR67890", "ORD11111", "SKU22222"
                ])
            }
        }).collect();
        ...
        group.bench_with_input(
            ...
            |b, columns| {
                ...
                b.to_async(rt).iter(|| async {
                    ranker.profile_columns(black_box(columns)).await.unwrap()
                })
            },
        );
        ...
        group.bench_with_input(
            ...
            |b, columns| {
                let rt = tokio::runtime::Runtime::new().unwrap();
                b.to_async(rt).iter(|| async {
                    ranker.build_similarity_graph(black_box(columns)).await.unwrap()
                })
            },
        );
    }
...
fn benchmark_feature_extraction_scalability(c: &mut Criterion) {
    ...
    for data_size in [100, 500, 1000, 2000].iter() {
        ...
        group.bench_with_input(
            ...
            &column,
            |b, column| {
                b.iter(|| extractor.extract_all_features(black_box(column)))
            },
        );
    }
...
fn benchmark_ml_classifier_batch_processing(c: &mut Criterion) {
    ...
    for batch_size in [50, 100, 500, 1000].iter() {
        let batch: Vec<Column> = (0..*batch_size).map(|i| {
            match i % 4 {
                0 => Column::new(&format!("text_col_{}", i), vec![
                    "Comprehensive product description with detailed specifications",
                    "Extensive user review with pros and cons analysis"
                ]),
                1 => Column::new(&format!("short_text_{}", i), vec![
                    "Good", "Bad", "Excellent", "Poor", "Average"
                ]),
                2 => Column::new(&format!("numeric_col_{}", i), vec![
                    "123.45", "67.89", "999.00"
                ]),
                _ => Column::new(&format!("id_col_{}", i), vec![
                    "USR12345", "ORD67890", "SKU11111"
                ]),
            }
        }).collect();
        ...
        group.bench_with_input(
            ...
            &batch,
            |b, batch| {
                b.iter(|| classifier.predict_batch(black_box(batch)))
            },
        );
    }
...
fn benchmark_graph_construction_performance(c: &mut Criterion) {
    ...
    c.bench_function("graph_construction_similar_columns", |b| {
        ...
        let rt = tokio::runtime::Runtime::new().unwrap();
        b.to_async(rt).iter(|| async {
            ranker.build_similarity_graph(black_box(&similar_columns)).await.unwrap()
        })
    });
    ...
    c.bench_function("graph_construction_diverse_columns", |b| {
        ...
        let rt = tokio::runtime::Runtime::new().unwrap();
        b.to_async(rt).iter(|| async {
            ranker.build_similarity_graph(black_box(&diverse_columns)).await.unwrap()
        })
    });
}
