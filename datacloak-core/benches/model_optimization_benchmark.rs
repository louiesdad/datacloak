...
use datacloak_core::model_optimization::{ModelOptimizer, ModelCache, QuantizationLevel};
...
fn benchmark_quantization_inference(c: &mut Criterion) {
    ...
    let quantized_int16 = optimizer.quantize(&model, QuantizationLevel::Int16).unwrap();
    let quantized_dynamic = optimizer.quantize(&model, QuantizationLevel::Dynamic).unwrap();
    ...
    group.bench_function("quantized_dynamic", |b| {
        b.iter(|| quantized_dynamic.predict(black_box(&test_features)).unwrap())
    });
