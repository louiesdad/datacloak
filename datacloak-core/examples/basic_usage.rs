...
use datacloak_core::{
    DataCloak, DataCloakConfig, DataSource, DataSourceConfig, Pattern, PatternSet, PatternType,
};
use std::env;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    config.llm_config.api_key =
        env::var("OPENAI_API_KEY").unwrap_or_else(|_| "your-api-key-here".to_string());
    ...
    patterns.add(
        Pattern::new(PatternType::Custom(1), r"\bEMP\d{6}\b".to_string())
    ...
    match datacloak
        .analyze_churn(memory_source, patterns.to_vec(), Some(10))
        .await
    {
        Ok(results) => {
            ...
            for prediction in results.predictions.iter().take(5) {
                println!(
                    "  Customer {}: {:.2}% churn probability (confidence: {:.2}%)",
                    prediction
                        .customer_id
                        .as_ref()
...
fn create_sample_csv() -> std::io::Result<()> {
    ...
    writeln!(
        file,
        "2,jane@test.com,555-987-6543,987-65-4321,4916338506082832,10.0.0.1,VIP customer"
    )?;
    writeln!(
        file,
        "3,bob@demo.org,555-456-7890,456-78-9012,4485393141463880,172.16.0.1,Frequent buyer"
    )?;
