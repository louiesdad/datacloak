...
impl ObfuscationCache {
    ...
    pub fn get_original(&self, token: &str) -> Option<String> {
        self.memory_cache.get(token).map(|entry| entry.original.clone())
    }
    ...
    pub async fn save(&self) -> Result<()> {
        ...
        // Collect all entries
        let entries: Vec<CacheEntry> = self.memory_cache
            .iter()
...
mod tests {
    ...
    fn test_cache_operations() {
        ...
        cache.store("TOKEN-1".to_string(), "john@example.com".to_string(), "EMAIL".to_string());
        
        assert_eq!(cache.get_original("TOKEN-1"), Some("john@example.com".to_string()));
        assert_eq!(cache.get_token("john@example.com"), Some("TOKEN-1".to_string()));
