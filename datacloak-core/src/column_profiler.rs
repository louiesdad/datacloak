...
impl ColumnProfiler {
    ...
    pub async fn profile_file(&self, _file_path: &str) -> Result<Vec<ColumnCandidate>, String> {
        ...
        let columns = vec![
            Column::new("description", vec!["Product description text", "Another long description"]),
            Column::new("price", vec!["19.99", "29.99", "39.99"]),
            Column::new("comments", vec!["Great product!", "Loved it", "Would buy again"]),
        ];
        
        // Use graph-based ranking with hybrid approach (70% ML, 30% graph)
        let candidates = self.graph_integration.rank_columns_with_graph(&columns, 0.7);
