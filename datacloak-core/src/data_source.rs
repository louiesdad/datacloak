...
use crate::{Result, DataCloakError};
...
impl DataSource {
    ...
    pub async fn sample(&self, max_records: usize) -> Result<Vec<serde_json::Value>> {
        match &self.config {
            DataSourceConfig::PostgreSQL { connection_string, query, .. } => {
                self.sample_postgres(connection_string, query, max_records).await
            }
            DataSourceConfig::CSV { path, delimiter, has_headers } => {
                self.sample_csv(path, *delimiter, *has_headers, max_records).await
            }
            DataSourceConfig::Memory { data } => {
                Ok(data.iter().take(max_records).cloned().collect())
            }
            _ => Err(DataCloakError::Other("Unsupported data source for sampling".into())),
        }
    }
    ...
    pub async fn stream(&self, batch_size: usize) -> Result<RecordStream> {
        match &self.config {
            DataSourceConfig::PostgreSQL { connection_string, query, .. } => {
                self.stream_postgres(connection_string, query, batch_size).await
            }
            DataSourceConfig::CSV { path, delimiter, has_headers } => {
                self.stream_csv(path, *delimiter, *has_headers, batch_size).await
            }
            DataSourceConfig::Memory { data } => {
                Ok(self.stream_memory(data.clone(), batch_size))
            }
            _ => Err(DataCloakError::Other("Unsupported data source for streaming".into())),
        }
    }
        ...
        let stream = futures::stream::unfold(
            ...
            |(client, cursor, batch_size)| async move {
                ...
                match client.query(&fetch_query, &[]).await {
                    ...
                    Err(e) => Some((Err(DataCloakError::Database(e.to_string())), (client, cursor, batch_size))),
                }
            },
        );
                        ...
                        Ok(file) => {
                            ...
                            for (i, result) in reader.deserialize().skip(offset).enumerate() {
                                ...
                                match result {
                                    ...
                                    Err(e) => return Some((Err(DataCloakError::Csv(e)), (path, delimiter, has_headers, batch_size, offset))),
                                }
                            }
                            ...
                            if batch.is_empty() {
                                ...
                                Some((Ok(batch), (path, delimiter, has_headers, batch_size, offset + batch_len)))
                            }
                        }
                        Err(e) => Some((Err(DataCloakError::Io(e)), (path, delimiter, has_headers, batch_size, offset))),
                    }
                }).await.ok()?
            },
        );
    ...
    fn postgres_value_to_json(&self, row: &tokio_postgres::Row, idx: usize) -> Result<serde_json::Value> {
        postgres_value_to_json_static(row, idx)
    }
}

// Helper function for PostgreSQL value conversion
fn postgres_value_to_json_static(row: &tokio_postgres::Row, idx: usize) -> Result<serde_json::Value> {
    ...
    match *column_type {
        ...
        _ => Ok(serde_json::Value::String(format!("{:?}", row.get::<_, Option<String>>(idx))))
    }
}
