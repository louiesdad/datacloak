                ...
                for edge in graph.edge_indices() {
                    if let Some((source, target)) = graph.edge_endpoints(edge) {
                        if target == node {
                            ...
                            let source_out_weight =
                                out_weights.get(&source).copied().unwrap_or(1.0);

                            if source_out_weight > 0.0 {
                                rank += self.damping_factor * ranks[&source] * edge_weight
                                    / source_out_weight;
                            }
                        }
                    }
...
impl ColumnGraph {
    ...
    pub fn calculate_pagerank_weighted(
        &self,
        damping_factor: f32,
        max_iterations: usize,
    ) -> HashMap<NodeIndex, f32> {
    ...
    pub fn get_top_nodes_by_pagerank(
        &self,
        k: usize,
        damping_factor: f32,
        max_iterations: usize,
    ) -> Vec<(NodeIndex, f32, String)> {
        let ranks = self.calculate_pagerank(damping_factor, max_iterations);
        let mut ranked_nodes: Vec<_> = ranks
            .into_iter()
            .filter_map(|(idx, rank)| {
