...
use crate::{Result, DataCloakError, ObfuscatedBatch, ObfuscatedRecord};
...
impl BatchLlmClient {
    ...
    pub fn new(config: LlmBatchConfig) -> Self {
        ...
        let rate_limiter = config.rate_limit.map(|rate| {
            Arc::new(tokio::sync::Mutex::new(RateLimiter::new(rate)))
        });
    ...
    async fn process_chunk(
        ...
        let batch_data = serde_json::to_string_pretty(&chunk)
            .map_err(DataCloakError::Serialization)?;
    ...
    async fn make_api_call(&self, request: &ChatRequest) -> Result<ChatResponse> {
        ...
        if !response.status().is_success() {
            ...
            return Err(DataCloakError::LlmApi(format!("API error {}: {}", status, body)));
        }
