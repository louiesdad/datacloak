...
impl MLGraphIntegration {
    ...
    pub fn build_column_graph(&self, columns: &[Column]) -> (ColumnGraph, Vec<ColumnCandidate>) {
        ...
        for column in columns {
            ...
            let embedding = self
                .feature_extractor
                .extract_header_embedding(&column.name);
    ...
    fn calculate_centrality_scores(
        ...
        for node_idx in graph.node_indices() {
            if let Some(node) = graph.get_node(node_idx) {
                ...
                let weighted_degree: f32 = neighbors
                    .iter()
    ...
    pub async fn profile_columns(
        &self,
        columns: &[Column],
    ) -> Result<Vec<ColumnCandidate>, String> {
...
mod tests {
    ...
    fn test_build_column_graph() {
        ...
        let columns = vec![
            Column::new(
                "description",
                vec!["Product description", "Another description"],
            ),
    ...
    fn test_rank_columns_with_graph() {
        ...
        let columns = vec![
            Column::new(
                "description",
                vec!["Long product description text", "Another paragraph"],
            ),
        ...
        assert!(
            ranked_ml[0].column_type == ColumnType::TextLong
    ...
    fn test_graph_centrality_calculation() {
        ...
        let columns = vec![
            ...
            Column::new(
                "customer_email",
                vec!["john@example.com", "jane@example.com"],
            ),
