...
impl PatternSet {
    ...
    pub fn default_pii() -> Self {
        ...
        ).with_description("Email addresses".to_string()).with_priority(100));
        
        set.add(Pattern::new(
            PatternType::SSN,
            r"\b\d{3}-\d{2}-\d{4}\b".to_string(),
        ).with_description("Social Security Numbers".to_string()).with_priority(200));
        
        set.add(Pattern::new(
            PatternType::Phone,
            r"\b\d{3}[-.]?\d{3}[-.]?\d{4}\b".to_string(),
        ).with_description("Phone numbers".to_string()).with_priority(90));
        
        set.add(Pattern::new(
            PatternType::CreditCard,
            r"\b4[0-9]{12}(?:[0-9]{3})?\b".to_string(),
        ).with_description("Credit card numbers (Visa)".to_string()).with_priority(150));
    ...
    pub fn hipaa_compliance() -> Self {
        ...
        ).with_description("Medical Record Numbers".to_string()).with_priority(180));
        
        set.add(Pattern::new(
            PatternType::DateOfBirth,
            r"\b\d{1,2}[/-]\d{1,2}[/-]\d{2,4}\b".to_string(),
        ).with_description("Dates of Birth".to_string()).with_priority(120));
