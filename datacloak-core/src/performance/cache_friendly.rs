...
impl CacheFriendlyGraph {
    ...
    pub fn iter_nodes_cache_friendly(&self) -> impl Iterator<Item = NodeId> + '_ {
        ...
        (0..max_id)
            .map(NodeId)
            .filter(move |id| self.nodes.contains_key(id))
    }
    ...
    pub fn batch_get_embeddings(&self, nodes: &[NodeId]) -> Vec<Option<Vec<f32>>> {
        nodes
            .iter()
            .map(|id| self.nodes.get(id).map(|n| n.embedding.clone()))
            .collect()
    }
    ...
    fn test_concurrent_access() {
        ...
        for i in 0..10 {
            ...
            let handle = thread::spawn(move || {
                for j in 0..100 {
                    graph_clone.add_node(format!("node_{}_{}", i, j), vec![i as f32, j as f32]);
                }
            });
            handles.push(handle);
        }
