...
impl MemoryPool {
    ...
    pub fn stats(&self) -> PoolStats {
        ...
        let allocated_bytes: usize = blocks.iter()
            .filter(|b| !b.is_free)
            .map(|b| b.size)
            .sum();
            
        let free_bytes: usize = blocks.iter()
            .filter(|b| b.is_free)
            .map(|b| b.size)
            .sum();
            
        let allocation_count = blocks.iter()
            .filter(|b| !b.is_free)
            .count();
    ...
    pub fn defragment(&self) {
        ...
        while i < blocks.len() - 1 {
            if blocks[i].is_free && blocks[i + 1].is_free {
                ...
                let end_of_current = unsafe {
                    blocks[i].ptr.as_ptr().add(blocks[i].size)
                };
