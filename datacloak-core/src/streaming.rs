...
use crate::{Result, DataCloakError, RecordBatch, ObfuscatedBatch, Obfuscator};
...
impl StreamProcessor {
        ...
        S: Stream<Item = Result<RecordBatch>> + Unpin + Send,
    {
        ...
        let _ = semaphore.acquire_many(self.config.max_concurrent_batches as u32).await;
    ...
    pub fn create_pipeline<S>(
        &self,
        input_stream: S,
    ) -> impl Stream<Item = Result<ObfuscatedBatch>>
    where
...
mod tests {
    ...
    async fn test_stream_processor() {
        ...
        // Create test stream
        let batch = vec![
            serde_json::json!({"email": "test@example.com", "name": "Test User"}),
        ];
