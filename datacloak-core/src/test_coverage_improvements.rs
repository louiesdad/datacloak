...
mod errors_tests {
    ...
    fn test_custom_error_creation() {
        ...
        assert_eq!(custom_error.to_string(), "Other error: Custom error message");
    }
    ...
    fn test_error_chaining() {
        ...
        // Convert to Result and test error propagation
        let result: Result<(), DataCloakError> = Err(datacloak_error);
...
#[cfg(test)]
mod llm_batch_tests {
    use crate::obfuscator::{ObfuscatedRecord, ObfuscatedChurnPrediction};
...
mod obfuscator_tests {
    ...
    fn test_obfuscator_basic() {
        let patterns = vec![Pattern::new(PatternType::Email, r"\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b".to_string())];
...
mod patterns_tests {
    ...
    fn test_pattern_creation() {
        let pattern = Pattern::new(PatternType::Email, r"\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b".to_string());
    ...
    fn test_pattern_with_description() {
        ...
        assert_eq!(pattern.description, Some("Social Security Number".to_string()));
    }
    
    #[test]
    fn test_pattern_with_priority() {
        let mut pattern = Pattern::new(PatternType::Phone, r"\b\d{3}[-.]?\d{3}[-.]?\d{4}\b".to_string());
    ...
    fn test_pattern_types() {
        ...
        for pattern_type in types {
            match pattern_type {
                ...
                PatternType::Custom(id) => assert!(format!("{:?}", pattern_type).contains(&id.to_string())),
