...
async fn test_streaming_processor() -> Result<()> {
    use datacloak_core::{Obfuscator, StreamConfig, StreamProcessor};
    use futures::stream;
...
async fn test_pattern_priority() -> Result<()> {
    ...
    let patterns = vec![
        ...
        .with_priority(100),
        Pattern::new(PatternType::Custom(1), r"\b\w+@\w+\.\w+\b".to_string()).with_priority(50),
    ];
