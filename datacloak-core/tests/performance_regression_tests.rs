...
mod regression_tests {
    ...
    fn test_graph_construction_performance() {
        ...
        println!(
            "Graph construction (1000 nodes): avg {}ms, times: {:?}",
            avg_time, times
        );
        ...
        assert!(
            avg_time < 5000,
            "Graph construction too slow: {}ms",
            avg_time
        );
        ...
        assert!(
            max_variance <= avg_time.max(2),
            "Performance too inconsistent: variance {}ms",
            max_variance
        );
    }
    ...
    fn test_pagerank_performance() {
        ...
        let avg_time = times.iter().sum::<u128>() / times.len() as u128;
        println!(
            "PageRank (1000 nodes): avg {}ms, times: {:?}",
            avg_time, times
        );
    ...
    fn test_similarity_computation_performance() {
        ...
        for &size in &[100, 300, 1024] {
            ...
            println!(
                "Cosine similarity ({}D): {:.2}µs per calculation",
                size, per_calculation
            );

            // Performance requirement: reasonable for debug mode
            assert!(
                per_calculation < 50.0,
                "Similarity calculation too slow for {}D: {:.2}µs",
                size,
                per_calculation
            );
        }
    ...
    fn test_memory_pool_performance() {
        ...
        println!(
            "Memory pool allocation: {:.2}µs per allocation ({} successful)",
            per_allocation,
            allocations.len()
        );
        ...
        assert!(
            per_allocation < 100.0,
            "Memory pool allocation too slow: {:.2}µs",
            per_allocation
        );
        assert!(
            allocations.len() > iterations / 10,
            "Too many allocation failures: {} out of {}",
            allocations.len(),
            iterations
        );
    }
    ...
    fn test_cache_friendly_graph_performance() {
        ...
        println!(
            "Cache-friendly graph - nodes: {:?}, edges: {:?}, traversal: {:?}",
        ...
        assert!(
            edge_creation_time.as_millis() < 500,
            "Edge creation too slow: {:?}",
            edge_creation_time
        );
        assert!(
            traversal_time.as_millis() < 50,
            "Traversal too slow: {:?}",
            traversal_time
        );
    ...
    fn test_simd_vs_scalar_performance() {
        ...
        #[cfg(feature = "similarity-search")]
        {
            ...
            println!(
                "SIMD dot product: {:.2}µs per operation, speedup: {:.2}x",
                simd_per_op, speedup
            );
        ...
        assert!(
            scalar_per_op < 50.0,
            "Scalar operations too slow: {:.2}µs",
            scalar_per_op
        );
    }
    ...
    fn test_memory_usage_bounds() {
        ...
        println!(
            "Memory usage - initial: {} MB, peak: {} MB, increase: {} MB",
    ...
    fn test_concurrent_performance() {
        ...
        println!(
            "Concurrent performance: {:.2}µs per operation ({} threads, {} ops/thread)",
        ...
        assert!(
            per_operation < 20.0,
            "Concurrent performance degradation: {:.2}µs",
            per_operation
        );
    }
    ...
    fn test_large_dataset_scalability() {
        ...
        for &size in &[100, 500, 1000, 2000] {
            ...
            assert!(
                construction_time.as_millis() < expected_max_construction.max(100),
                "Construction doesn't scale for size {}: {}ms",
                size,
                construction_time.as_millis()
            );
            assert!(
                pagerank_time.as_millis() < expected_max_pagerank.max(50),
                "PageRank doesn't scale for size {}: {}ms",
                size,
                pagerank_time.as_millis()
            );
        }
    }
}
